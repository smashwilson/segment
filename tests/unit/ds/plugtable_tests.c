...
static void test_access(void)
{
  ...
  seg_plugtable_put(table, &onek, value);
...
static void test_putifabsent(void)
{
  ...
  seg_plugtable_put(table, &key0, value0);
  CU_ASSERT_EQUAL(seg_plugtable_count(table), 1);

  /* Insert a new item. */
  void *existing1 = seg_plugtable_putifabsent(table, &key1, value1);
  CU_ASSERT_PTR_EQUAL(existing1, value1);
  CU_ASSERT_EQUAL(seg_plugtable_count(table), 2);

  /* Retrieve an existing item. */
  void *existing0 = seg_plugtable_putifabsent(table, &key0, value2);
